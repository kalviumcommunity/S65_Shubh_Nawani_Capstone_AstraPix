...
const ImageCard = ({ image, onDelete }) => {
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(
                `${import.meta.env.VITE_BASE_URI}/generate/${image._id}`,
                {
                    headers: {
    ...
    const handleShare = async () => {
        try {
            ...
            const file = new File([blob], `astrapix-image-${Date.now()}.jpg`, {
                type: "image/jpeg",
            });
    ...
    const thumbnailUrl = useMemo(
        () =>
            image.imageUrl.replace(
                "/upload/",
                "/upload/w_400,f_auto,q_auto:eco/",
            ),
        [image.imageUrl],
    );
        ...
        >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 touch:opacity-100">
                <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-3">
                    <p className="text-white text-xs sm:text-sm line-clamp-2 mb-1.5 sm:mb-2">
                        {image.prompt}
                    </p>
